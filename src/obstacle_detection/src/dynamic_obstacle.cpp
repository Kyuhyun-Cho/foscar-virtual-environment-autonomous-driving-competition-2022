...
ros::Publisher dyObsClusterPub; //Cluster Publishser
ros::Publisher dyObsMarkerPub; //Bounnding Box Visualization Publisher
ros::Publisher dyObsPosePub; //Bounding Box Position Publisher
ros::Publisher dyObsCropboxPub; //Cropbox Publishser
...
void cloud_cb(const sensor_msgs::PointCloud2 inputcloud) {
  ...
  dyObsClusterPub.publish(cluster);
  dyObsMarkerPub.publish(BoxArray);
  dyObsPosePub.publish(BoxPosition);
  dyObsCropboxPub.publish(cropbox);
...
int main(int argc, char **argv) {
  ...
  dyObsClusterPub = nh.advertise<sensor_msgs::PointCloud2>("/dy_obs_cluster", 0.001);                  
  dyObsMarkerPub = nh.advertise<visualization_msgs::MarkerArray>("/dy_obs_marker", 0.001);  
  dyObsPosePub = nh.advertise<obstacle_detection::Boundingbox>("/dy_obs_position", 0.001);    
  dyObsCropboxPub = nh.advertise<sensor_msgs::PointCloud2>("/dy_obs_cropbox", 0.001); 
