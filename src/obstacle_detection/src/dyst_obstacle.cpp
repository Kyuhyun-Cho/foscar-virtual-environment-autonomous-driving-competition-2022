...
ros::Publisher dystObsClusterPub; //Cluster Publishser
ros::Publisher dystObsMarkerPub; //Bounnding Box Visualization Publisher
ros::Publisher dystObsPosePub; //Bounding Box Position Publisher
ros::Publisher dystObsCropboxPub; //Cropbox Publishser

laser_geometry::LaserProjection projector_;
// tf::TransformListener listener_;

void dynamicParamCallback(obstacle_detection::dyst_hyper_parameterConfig &config, int32_t level) {
...
void cloud_cb(const sensor_msgs::PointCloud2 inputcloud) {
  ...
  dystObsClusterPub.publish(cluster);
  dystObsMarkerPub.publish(BoxArray);
  dystObsPosePub.publish(BoxPosition);
  dystObsCropboxPub.publish(cropbox);
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "dyst_obstacle");
  ros::NodeHandle nh;

  dynamic_reconfigure::Server<obstacle_detection::dyst_hyper_parameterConfig> server;
  dynamic_reconfigure::Server<obstacle_detection::dyst_hyper_parameterConfig>::CallbackType f;
  ...
  dystObsClusterPub = nh.advertise<sensor_msgs::PointCloud2>("/dyst_obs_cluster", 0.001);                  
  dystObsMarkerPub = nh.advertise<visualization_msgs::MarkerArray>("/dyst_obs_marker", 0.001);  
  dystObsPosePub = nh.advertise<obstacle_detection::Boundingbox>("/dyst_obs_position", 0.001);    
  dystObsCropboxPub = nh.advertise<sensor_msgs::PointCloud2>("/dyst_obs_cropbox", 0.001); 
