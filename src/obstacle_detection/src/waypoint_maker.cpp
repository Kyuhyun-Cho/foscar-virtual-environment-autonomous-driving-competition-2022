...
double pivot_reset_area[4] = {-6.0, 4.80, -3.5, 5.6};
...
class WaypointMaker{
    // attribute
    ObjectArray objects; // subscribe msg data ( obstacle_detection 토픽 구독해서 받는 정보를 저장하는 객체 )
    ...
    ConeArray rightCones; // cone segmentation ( 우측 cone의 정보를 담는 객체 )
    ...
    */
    ...
    obstacle_detection::Waypoint waypointInfoMsg; // waypoint msg ( Waypoint를 발행하기 위한 메시지타입 객체 )
    ...
    WaypointMaker() {
        ...
        egoSub = nh.subscribe("/Ego_topic", 1, &WaypointMaker::egoCallback, this);
        mainSub = nh.subscribe("/tr_obs_position", 1, &WaypointMaker::mainCallback, this);

        visualizeConePub = nh.advertise<visualization_msgs::MarkerArray>("/cone_marker", 0.001);
        waypointInfoPub = nh.advertise<obstacle_detection::Waypoint>("/waypoint_info", 0.001);
    ...
    // method
    void mainCallback(const obstacle_detection::ObjectInfo& msg); // 이 객체의 전체 로직을 수행하는 함수입니다.
    void egoCallback(morai_msgs::EgoVehicleStatus msg);
    void setObjectInfo(const obstacle_detection::ObjectInfo& msg); // object_info 정보를 받아 값을 objects에 저장하고 X값 기준 오름차순으로 정렬합니다.
    ...
    // dynamic reconfigure
    // void cfgCallback(obstacle_detection::wp_hyper_parameterConfig &config, int32_t level);
};

void WaypointMaker::mainCallback(const obstacle_detection::ObjectInfo& msg) {
...
void WaypointMaker::egoCallback(morai_msgs::EgoVehicleStatus data) {
    
    double pos_x = data.position.x;
    double pos_y = data.position.y;
...
void WaypointMaker::setObjectInfo(const obstacle_detection::ObjectInfo& msg) {
    /**
     * @brief 
...
void WaypointMaker::visualizeLeftRightCone() {
    ...
    coneObject.scale.x = 0.15;
    coneObject.scale.y = 0.15;
    coneObject.scale.z = 0.01;
...
void WaypointMaker::visualizePivot() {
    ...
    pivotObject.scale.x = 0.15;
    pivotObject.scale.y = 0.15;
    pivotObject.scale.z = 0.3;
...
void cfgCallback(obstacle_detection::wp_hyper_parameterConfig &config, WaypointMaker* wm) {
...
int main(int argc, char **argv) {
    ...
    dynamic_reconfigure::Server<obstacle_detection::wp_hyper_parameterConfig> server;
    dynamic_reconfigure::Server<obstacle_detection::wp_hyper_parameterConfig>::CallbackType f;
